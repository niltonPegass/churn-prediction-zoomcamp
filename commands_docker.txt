...
1. sudo docker run -it --rm churn-prediction:latest
...
2. sudo docker run -it --rm --entrypoint=bash churn-prediction:latest
...
docker build -t <container_name_service> .
docker build -t churn-prediction . #niltonpegass


## RUNNING A DOCKER IMAGE

docker run -it --rm -p 2909:2909 churn-prediction:latest
